 ...
 * @author Anuradha
 */
public class TabBaseGroupLayout implements GroupEditorLayout {

    private TabGroupView leftSideView;
    private TabGroupView rightSideView;
    ...
    private void buildLayout(JPanel container, TabGroupView view, Group... groups) {
        container.removeAll();
        if (groups.length > 0) {
            if (view == null) {
                view = new TabGroupView();
            } else {
                view.clear();
            }
    ...
    private void setActiveGroup(TabGroupView view, Group group) {
        if (view != null) {
